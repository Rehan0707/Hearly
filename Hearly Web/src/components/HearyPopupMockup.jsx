...
const HearyPopupMockup = ({ screen, enrolledProfile, meetings }) => {
  ...
  const displayMeetings = (meetings && meetings.length > 0)
    ? Array.from(new Set(meetings.map(m => m.sessionId))).map((sessId, index) => {
        const sessMessages = meetings.filter(m => m.sessionId === sessId);
        const lastMsg = sessMessages[sessMessages.length - 1];
        const dateStr = new Date(lastMsg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        const summaryText = sessMessages.map(m => `[${m.speaker === 'you' ? 'You' : 'Others'}]: ${m.text}`).join(' ');
        return {
          id: sessId,
          title: `Meeting Session ${index + 1}`,
          time: dateStr,
          status: "Analyzed",
          speakers: Array.from(new Set(sessMessages.map(m => m.speaker))).length,
        ...
        {screen === 'enrollment' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', textAlign: 'center' }}>
            ...
            <h3 style={{ fontSize: '1.3rem', color: '#FFFFFF', fontFamily: 'var(--font-body)' }}>
              {enrolledProfile ? 'Speaker Enrolled' : (enrollState === 'success' ? 'Speaker Enrolled' : 'Enrolling Speaker')}
            </h3>
            <p style={{
              fontSize: '0.88rem',
              ...
              {enrolledProfile ? `Speaker profile for "${enrolledProfile.name}" is active.` : (enrollState === 'success' ? 'Your voice print has been securely saved.' : 'Say "Hello Hearly" to start the voice identification process.')}
            </p>
            <div style={{
              display: 'flex',
              gap: '8px',
              ...
              <button 
                ...
                disabled={enrolledProfile !== null || enrollState !== 'idle'}
                style={{
                flex: 1,
                background: (enrolledProfile || enrollState === 'success') ? '#10B981' : 'var(--brand-crimson)',
                color: '#050505',
                border: 'none',
                padding: '14px',
                ...
                fontWeight: 700,
                fontSize: '0.9rem',
                cursor: (enrolledProfile || enrollState !== 'idle') ? 'default' : 'pointer',
                opacity: enrollState === 'loading' ? 0.7 : 1,
                transition: 'all 0.3s ease',
              }}>
                {enrolledProfile ? 'Enrolled via Extension' : (enrollState === 'loading' ? 'Processing...' : enrollState === 'success' ? 'Done' : 'Confirm Voice')}
              </button>
              <button 
